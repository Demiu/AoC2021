use std::{collections::HashSet, ops::RangeInclusive, cmp::{max, min}};

use anyhow::{Result, anyhow};
use itertools::Itertools;
use nom::{sequence::separated_pair, bytes::complete::tag, IResult, multi::separated_list1};
...
pub fn parse_input(file: &[u8]) -> Result<SolverInput> {
    let mut lines = separated_list1(tag(b"\n"), parse_line)(file).map_err(|_| anyhow!("Failed parsing lines")).map(move |t| t.1)?;
    for line in lines.iter_mut() {
        if let Line::Generic{start, end} = line {
            match (start.x == end.x, start.y == end.y) {
                (true, true) =>  *line = Line::PointLike(Point {x:start.x,y:start.y}),
                (false, true) => *line = if start.x <= end.x {
                    Line::Horizontal{ xs: start.x..=end.x, y: start.y }
                } else {
                    Line::Horizontal{ xs: end.x..=start.x, y: start.y }
                },
                (true, false) => *line = if start.y <= end.y {
                    Line::Vertical{ x: start.x, ys: start.y..=end.y }
                } else {
                    Line::Vertical{ x: start.x, ys: end.y..=start.y }
                },
                _ => (),
            }
        } else { unreachable!() }
...
pub fn solve_part1(input: &SolverInput) -> u32 {
    fn range_intersect<Idx: Ord+Copy>(l: &RangeInclusive<Idx>, r: &RangeInclusive<Idx>) -> Option<RangeInclusive<Idx>> {
    ...
    for (line1, line2) in input.iter().tuple_combinations() {
        match (line1, line2) {
            (Line::Vertical{x: x1, ys: ys1}, Line::Vertical{x: x2, ys: ys2}) => if x1 == x2 {
            ...
            (Line::Horizontal{xs: xs1, y: y1}, Line::Horizontal{xs: xs2, y: y2}) => if y1 == y2 {
            ...
            (Line::Horizontal{xs, y}, Line::Vertical{x,ys}) => if xs.contains(x) && ys.contains(y) {
                positions.insert( Point {x: *x, y: *y} );
            },
            (Line::Vertical{x,ys}, Line::Horizontal{xs, y}) => if xs.contains(x) && ys.contains(y) {
                positions.insert( Point {x: *x, y: *y} );
            }
            _ => (),
        }
