...
mod test {
    ...
    fn step_example_small() {
        let mut parsed = crate::macros::parse_expect!(EXAMPLE_SMALL, "small example");
    ...
    fn step_example_large() {
        let mut parsed = crate::macros::parse_expect!(EXAMPLE_LARGE, "large example");
        // cherrypick lines
        step(&mut parsed);
        assert_eq!(
            parsed.index_axis(Axis(0), 1),
    ...
    crate::macros::make_test_for_day!(example, EXAMPLE_LARGE, 1656, 195);
}
